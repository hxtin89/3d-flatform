...
export function createAudioLayer(options: AudioLayerOptions): AudioLayer {
  ...
  function paintButton(message?: string): void {
    ...
    toggle.setAttribute(
      'aria-label',
      enabled ? 'Naturklänge ausschalten' : 'Naturklänge einschalten',
    )
    status.textContent = message
      ?? (enabled ? 'Naturklänge aktiviert' : 'Naturklänge stumm')
  }
  ...
  function initialize(): void {
    ...
    for (const track of tracks) {
      ...
      context.createMediaElementSource(track.element)
        .connect(track.gain)
        .connect(master)
      track.element.load()
    }
  ...
  function ramp(track: Track, target: number, seconds: number): void {
    if (!context || !track.gain
      || Math.abs(track.lastTarget - target) < 0.008) return
  ...
  function applyMix(): void {
    ...
    const start = degToRad(EXPERIENCE_CONFIG.audio.nightBlendEndDeg)
    const end = degToRad(EXPERIENCE_CONFIG.audio.nightBlendStartDeg)
    const dayMix = smoothstep(start, end, currentDaylight.sunElevationRad)
    ramp(
      day,
      EXPERIENCE_CONFIG.audio.ambientVolume * dayMix,
      EXPERIENCE_CONFIG.audio.daylightFadeSeconds,
    )
    ramp(
      night,
      EXPERIENCE_CONFIG.audio.ambientVolume * (1 - dayMix),
      EXPERIENCE_CONFIG.audio.daylightFadeSeconds,
    )
    ramp(
      rain,
      currentRainActive ? EXPERIENCE_CONFIG.audio.rainVolume : 0,
      EXPERIENCE_CONFIG.audio.weatherFadeSeconds,
    )
  }
