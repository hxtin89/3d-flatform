...
- `adaptive-quality.ts` — picks the screen-space error target from camera distance alone: a three-rung ladder (APH 4/8/16, One-LOD 64/124/256) with hysteresis on the band edges. Deliberately open loop — there is no frame-time feedback, and `main.ts` computes the distance as the slant range to the ground ahead so the target follows the view angle as well as the height.
